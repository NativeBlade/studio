...
- Personalize the loading splash at \`resources/js/index.html\` so the app never shows NativeBlade's default branding: replace the \`/logo_nb.png\` logo and the demo colors with something that fits ${appInfo.name} (an emoji or simple inline SVG mark is fine — no external image needed), keep the app's name, and match the visual style the user chose. Keep the loading MECHANICS intact — do NOT remove or rename \`#splash\`, \`#app\`, or the two \`<script>\` tags (app.js reads these by id). The \`.spinner\` and \`#status\` elements are OPTIONAL boot decoration — safe to remove or restyle (\`#status\` is read null-safely). A minimal splash = your branding inside \`#splash\` + the \`#app\` iframe + the scripts.
- Commit a meaningful git checkpoint after each round of changes so work can be rolled back (the repo has a local identity set; if git is unavailable on this machine, skip checkpoints silently — never bother the user about it).
