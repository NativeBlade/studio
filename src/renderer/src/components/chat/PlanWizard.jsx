            ...
            <div style={{ flex: 1, borderRadius: 16, padding: 16, background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,77,77,0.28)', opacity: approved ? 0.6 : 1, pointerEvents: approved ? 'none' : 'auto' }}>
                        ...
                        {questions.map((q, qi) => (
                            <div key={qi}>
                                <div style={{ marginBottom: 6, fontSize: 12.5, fontWeight: 600, color: '#e7e9ee' }}>
                                    {q.question}{q.multi && <span style={{ marginLeft: 6, fontSize: 10.5, fontWeight: 400, color: '#6b7280' }}>select all that apply</span>}
                                </div>
                                ...
                                <div style={{ marginTop: 6, display: 'flex', alignItems: 'center', gap: 8, borderRadius: 10, padding: '6px 12px', background: (other[qi] ?? '').trim() ? 'rgba(220,38,38,0.1)' : 'rgba(255,255,255,0.03)', border: `1px solid ${(other[qi] ?? '').trim() ? 'rgba(255,77,77,0.4)' : 'rgba(255,255,255,0.1)'}` }}>
                                    <span style={{ flexShrink: 0, fontSize: 12, fontWeight: 500, color: '#9aa0a8' }}>Other:</span>
                                    <input value={other[qi] ?? ''} onChange={(e) => setOther((p) => ({ ...p, [qi]: e.target.value }))} placeholder="type your own…" style={{ width: '100%', background: 'transparent', border: 'none', outline: 'none', fontSize: 12.5, color: '#e7e9ee' }} />
                                </div>
                            </div>
                        ))}
                    </div>
                ...
                {label === 'Review' && !changing && (
                    <div style={{ marginBottom: 12, display: 'flex', flexDirection: 'column', gap: 10 }}>
                        <div>
                            <div style={{ marginBottom: 4, fontSize: 11, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.08em', color: '#9aa0a8' }}>Features ({selected.length})</div>
                            {selected.map((st, i) => <div key={i} style={{ fontSize: 12.5, color: '#c2c7cf' }}>• <span dangerouslySetInnerHTML={{ __html: mdToHtml(st) }} /></div>)}
                        </div>
                        {answers.length > 0 && (
                            <div>
                                <div style={{ marginBottom: 4, fontSize: 11, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.08em', color: '#c4b5fd' }}>Design & preferences</div>
                ...
                {label === 'Review' && changing ? (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                        <textarea value={feedback} onChange={(e) => setFeedback(e.target.value)} rows={2} placeholder="What should change in the plan?" className="nb-field" style={{ width: '100%', resize: 'none', borderRadius: 10, padding: '8px 12px', fontSize: 13 }} />
                        <div style={{ display: 'flex', gap: 8 }}>
                            <button onClick={() => onReject(feedback)} disabled={!feedback.trim()} className="nb-btn" style={{ borderRadius: 10, padding: '6px 14px', fontSize: 12.5, fontWeight: 600, color: '#fff', border: 'none', background: feedback.trim() ? 'linear-gradient(180deg,#8b5cf6,#6d28d9)' : 'rgba(255,255,255,0.06)' }}>Re-plan</button>
                            <button onClick={() => setChanging(false)} className="nb-btn" style={{ borderRadius: 10, padding: '6px 12px', fontSize: 12.5, background: 'none', border: 'none', color: '#9aa0a8' }}>Cancel</button>
                        </div>
                    </div>
                ) : (
                    <div style={{ display: 'flex', gap: 8 }}>
                        {stage > 0 && <button onClick={() => setStage(stage - 1)} className="nb-btn" style={{ whiteSpace: 'nowrap', borderRadius: 10, padding: '6px 14px', fontSize: 12, fontWeight: 500, background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.14)', color: '#c2c7cf' }}>Back</button>}
                        {label !== 'Review' ? (
                            <button onClick={() => setStage(stage + 1)} disabled={!canNext} className="nb-btn" style={{ whiteSpace: 'nowrap', borderRadius: 10, padding: '6px 14px', fontSize: 12, fontWeight: 600, color: '#fff', border: 'none', background: canNext ? 'linear-gradient(180deg,#ff5151,#d31f1f)' : 'rgba(255,255,255,0.06)' }}>Next</button>
                        ) : (
                            <>
                                <button onClick={() => onApprove(selected, answers)} disabled={!selected.length} className="nb-btn" style={{ whiteSpace: 'nowrap', borderRadius: 10, padding: '6px 14px', fontSize: 12, fontWeight: 600, color: '#fff', border: 'none', background: selected.length ? 'linear-gradient(180deg,#ff5151,#d31f1f)' : 'rgba(255,255,255,0.06)' }}>Approve & build</button>
                                <button onClick={() => setChanging(true)} className="nb-btn" style={{ whiteSpace: 'nowrap', borderRadius: 10, padding: '6px 14px', fontSize: 12, fontWeight: 500, background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.14)', color: '#c2c7cf' }}>Request changes</button>
                            </>
                        )}
                    </div>
                )}
